...
@Component({
  ...
  template: `
    ...
    <p *ngIf="!children().length" class="text-mos-muted">No children linked to your account.</p>
    <app-card *ngFor="let c of children()" class="block mb-6">
      <h3 class="text-2xl font-bold text-white mb-4">{{ c.student.name }}</h3>
      <div class="grid md:grid-cols-3 gap-4 mb-4">
        <div>
          <p class="text-mos-muted text-sm uppercase mb-2">Attendance</p>
          <p *ngFor="let a of c.attendance" class="text-mos-text text-sm">{{ a.status }}: {{ a.count }}</p>
        </div>
        <div>
          <p class="text-mos-muted text-sm uppercase mb-2">Fees</p>
          <p *ngFor="let f of c.fees" class="text-sm" [class.text-red-400]="f.status === 'Unpaid'" [class.text-mos-text]="f.status !== 'Unpaid'">
            £{{ f.amount }} — {{ f.status }} (due {{ f.dueDate }})
          </p>
        </div>
        <div>
          <p class="text-mos-muted text-sm uppercase mb-2">Progress</p>
          <p *ngFor="let n of c.progressNotes" class="text-mos-text text-sm mb-2">{{ n.note }}</p>
        </div>
      </div>
    </app-card>
  `
